...
impl IconOperationConfig for BitmaskSliceReconstruct {
    ...
    fn perform_operation(
        ...
        let ignored_states = frames_drop_prefix.into_iter().filter_map(|(_, suffix)| {
            if suffix.parse::<i32>().is_ok() || strings_caught.iter().any(|caught| *caught == suffix) {
                None
            } else {
                Some(format!("({suffix})"))
            }
        ...
        if let Some(missed_suffixes) = ignored_states {
            ...
            return Err(ProcessorError::DmiError(
                format!("Restoration would fail to properly parse the following icon states [{missed_suffixes}] not parsed like [{caught_text}]")
            ));  
        }
        ...
        for (x, state) in trimmed_frames.into_iter().enumerate() {
            ...
            for (y, frame) in state.images.into_iter().enumerate() {
                ...
                output_image.copy_from(&frame, (x as u32) * icon.width, (y as u32) * icon.height).unwrap_or_else(|_| panic!("Failed to copy frame (bad dmi?): {} #{}", state.name, y));
            }
        }
